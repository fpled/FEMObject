function [u,result] = dsolve_power(GSD,T,b,A,B,u0,ureuse,varargin)
% function [u,result] = dsolve_power(GSD,T,b,A,B,u0,ureuse,varargin)
% fonction solve GSD : resolution de A dt(u) + B u = b
% A B et b : PCMATRIX ou PCRADIALMATRIX
...
for j=1:nbfoncmax %%%%%%%%%%%%%%%% BOUCLE NOUVELLES FONCTIONS
    ...
    if ~strcmp(errorindicator,'none') && result.error(j)<paramradial.tol
        if strcmp(errorindicator,'rayleigh')
            u = normsort(u);
