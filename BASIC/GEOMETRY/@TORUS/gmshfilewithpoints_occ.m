...
if nargin<5 || isempty(numberembeddedpoints)
    if extrusion
        numberembeddedpoints = 2+(1:length(P));
    else
        if isfull, numberembeddedpoints = 1+(1:length(P));
        else,      numberembeddedpoints = 2+(1:length(P));
        end
    end
end
    ...
    if isfull
        ...
        if ~ischarin('recombine',varargin)
            ...
            if ischarin('recombine',varargin) && ~ischarin('Layers',varargin)
                numberlayers = max(1, round((radii(1)*pi)/clT));
        ...
        if ischarin('recombine',varargin) && ~ischarin('Layers',varargin) && ~isempty(angle_val)
            numberlayers = max(1, round((radii(1)*angle_val)/clT));
            varargin = [varargin, {'Layers',numberlayers}];
        end
