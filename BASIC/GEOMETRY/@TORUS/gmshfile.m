function varargout = gmshfile(T,cl,numbercenter,numberpoints,numbercurves,numbercurveloops,numbersurfaces,numbersurfaceloop,numbervolume,varargin)
% function G = gmshfile(T,cl,numbercenter,numberpoints,numbercurves,numbercurveloops,numbersurfaces,numbersurfaceloop,numbervolume)
% T : TORUS
...
if extrusion && ~isempty(angle_val) && angle_val <= tol
    error('gmshfile:degenerateAngle', 'Angle must be > 0 for extrusion.');
end

% Generate all vertices (minor circle profile points)
P = getvertices(T);
n = numel(P)/4; % number of minor-circle rings in non-extruded mode

if extrusion
    if nargin<3 || isempty(numbercenter), numbercenter = 1:2; end                     % 1 major center + 1 minor center
    if nargin<4 || isempty(numberpoints), numberpoints = max(numbercenter)+(1:4); end % 4 points for the minor circle
    if nargin<5 || isempty(numbercurves), numbercurves = 1:4; end                     % 4 arcs for the minor circle
    if nargin<6 || isempty(numbercurveloops), numbercurveloops = 1; end               % 1 minor circle
else
    if nargin<3 || isempty(numbercenter), numbercenter = 1:(n+3); end                   % 3 major centers + n minor centers
    if nargin<4 || isempty(numberpoints), numberpoints = max(numbercenter)+(1:4*n); end % 4 points for each of the n minor circles
...
if extrusion
    ...
    G = createpoint(G,center_minor,cl,numbercenter(2));
    ...
    if isfull
        ...
        if ischarin('recombine',varargin) && ~ischarin('Layers',varargin)
            numberlayers = max(1, round((radius*pi/2)/cl));
        ...
        if ~isempty(ang_val) && abs(ang_val) <= tol
            ...
            if ischarin('recombine',varargin) && ~ischarin('Layers',varargin) && ~isempty(ang_val)
                numberlayers = max(1, round((radius*ang_val)/cl));
                varargin = [varargin, {'Layers',numberlayers}];
            end
        ...
        if ~ischarin('recombine',varargin)
            ...
            numbervolume = numbervolumes;
            G = recombinesurface(G,[{numbersurface,numbertopsurface}, numbertopsurfaces]);
    ...
    G = createpoint(G,center,cl,numbercenter(1));
    G = createpoints(G,centers_major,cl,numbercenter(2:3));
    G = createpoints(G,centers_minor,cl,numbercenter(4:end));
