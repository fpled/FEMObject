...
if dim==1
    ...
    if strcmpi(split,'stress')
        ...
        if ischarin('+',varargin)
            ...
            Cm = Pm'/D*Pm + Pp'/D*Pm + Pm'/D*Pp; % undamaged part of compliance operator in Voigt notation
        else
            % undamaged negative part only
            Cp = Pp'/D*Pp + Pp'/D*Pm + Pm'/D*Pp; % damaged part of compliance operator in Voigt notation
