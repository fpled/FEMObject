function mov = evolTotalSobolIndicesGlobalLocalSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,alpha,varargin)
% function mov = evolTotalSobolIndicesGlobalLocalSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,alpha,varargin)
% Display evolution of the Total Sobol indices of reference global solution Ut_ref and local solution wt_ref
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Total Sobol index of U_ref_' num2str(i) ' and w_ref_' num2str(i) ' over complementary subdomain and patches for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Total Sobol index of U_ref_' num2str(i) ' and w_ref_' num2str(i) ' over complementary subdomain and patches for random variables #' num2str(alpha)])
else
    figure('Name',['Total Sobol index of U_ref and w_ref over complementary subdomain and patches for random variables #' num2str(alpha)])
...
swt_ref = cellfun(@(patch) TIMEMATRIX(reshape(SensitivityAnalysis.totalSobolIndices(wt_ref{patch.number},alpha,ndims(wt_ref{patch.number})),wt_ref{patch.number}.sz),T),patches.patches,'UniformOutput',false);
