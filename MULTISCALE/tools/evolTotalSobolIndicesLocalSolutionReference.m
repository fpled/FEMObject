function mov = evolTotalSobolIndicesLocalSolutionReference(patches,T,wt_ref,alpha,varargin)
% function mov = evolTotalSobolIndicesLocalSolutionReference(patches,T,wt_ref,alpha,varargin)
% Display evolution of the Total Sobol indices of reference local solution wt_ref
...
elseif isa(patches,'Patch')
    ...
    if ischarin('displ',varargin)
        i = getcharin('displ',varargin);
        figure('Name',['Total Sobol index of w_ref_' num2str(i) ' over patch #' num2str(patch.number) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of w_ref_' num2str(i) ' over patch #' num2str(patch.number) ' for random variables #' num2str(alpha)])
    else
        figure('Name',['Total Sobol index of w_ref over patch #' num2str(patch.number) ' for random variables #' num2str(alpha)])
