function mov = evolClosedSobolIndicesGlobalLocalSolution(glob,patches,interfaces,T,Ut,wt,alpha,varargin)
% function mov = evolClosedSobolIndicesGlobalLocalSolution(glob,patches,interfaces,T,Ut,wt,alpha,varargin)
% Display evolution of the Closed Sobol indices of global solution Ut and local solution wt
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Closed Sobol index of U_' num2str(i) ' and w_' num2str(i) ' over domain and patches for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Closed Sobol index of U_' num2str(i) ' and w_' num2str(i) ' over domain and patches for random variables #' num2str(alpha)])
else
    figure('Name',['Closed Sobol index of U and w over domain and patches for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Closed Sobol index of U and w over domain and patches for random variables #' num2str(alpha)])
end
clf
set(gcf,'Color','w')
...
swt = cellfun(@(patch) TIMEMATRIX(reshape(SensitivityAnalysis.closedSobolIndices(wt{patch.number},alpha,ndims(wt{patch.number})),wt{patch.number}.sz),T),patches.patches,'UniformOutput',false);
