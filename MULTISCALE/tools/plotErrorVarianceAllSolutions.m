function plotErrorVarianceAllSolutions(glob,patches,interfaces,U,w,lambda,U_ref,w_ref,lambda_ref,varargin)
% function plotErrorVarianceAllSolutions(glob,patches,interfaces,U,w,lambda,U_ref,w_ref,lambda_ref,varargin)
...
for k=1:n
    ...
    err_patch{k} = abs(variance(w{patch.number})-variance(w_ref{patch.number}))'./max(variance(w{patch.number}));
...
for k=1:n
    ...
    subplot(1+n,2,2*(k+1))
    interface = interfaces.interfaces{k};
    err_interface = abs(variance(lambda{interface.number})-variance(lambda_ref{interface.number}))'./max(variance(lambda{interface.number}));
