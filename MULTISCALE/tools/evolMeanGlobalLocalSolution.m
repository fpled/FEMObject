function mov = evolMeanGlobalLocalSolution(glob,patches,interfaces,T,Ut,wt,varargin)
% function mov = evolMeanGlobalLocalSolution(glob,patches,interfaces,T,Ut,wt,varargin)
% Display evolution of the mean (mathematical expectation) of global solution Ut and local solution wt
% glob: Global
% patches: Patches
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Mean of U_' num2str(i) ' and w_' num2str(i) ' over domain and patches'])
    % set(gcf,'Name',['Mean of U_' num2str(i) ' and w_' num2str(i) ' over domain and patches'])
else
    figure('Name','Mean of U and w over domain and patches')
...
wt = cellfun(@(patch) TIMEMATRIX(reshape(mean(wt{patch.number}),wt{patch.number}.sz),T),patches.patches,'UniformOutput',false);
