function mov = evolVarianceGlobalLocalSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,varargin)
% function mov = evolVarianceGlobalLocalSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,varargin)
% Display evolution of the variance of reference global solution Ut_ref and local solution wt_ref
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Variance of U_ref_' num2str(i) ' and w_ref_' num2str(i) ' over complementary subdomain and patches'])
    % set(gcf,'Name',['Variance of U_ref_' num2str(i) ' and w_ref_' num2str(i) ' over complementary subdomain and patches'])
else
    figure('Name','Variance of U_ref and w_ref over complementary subdomain and patches')
    % set(gcf,'Name','Variance of U_ref and w_ref over complementary subdomain and patches')
end
clf
set(gcf,'color','w')
...
vwt_ref = cellfun(@(patch) TIMEMATRIX(reshape(variance(wt_ref{patch.number}),wt_ref{patch.number}.sz),T),patches.patches,'UniformOutput',false);
...
if p.Results.rescale
    ...
    for j=1:n
        ...
        if ischarin('sigma',varargin)
            q = calc_sigma(patch.S,vwt_ref{patch.number},varargin{2:end});
            k = getcharin('sigma',varargin);
            q = sigmacompo(q,k,patch.S);
        elseif ischarin('epsilon',varargin)
            q = calc_epsilon(patch.S,vwt_ref{patch.number},varargin{2:end});
            k = getcharin('epsilon',varargin);
            q = sigmacompo(q,k,patch.S);
        elseif ischarin('energyint',varargin)
            q = calc_energyint(patch.S,vwt_ref{patch.number},varargin{2:end});
        elseif ischarin('displ',varargin)
            q = unfreevector(patch.S,vwt_ref{patch.number});
            k = getcharin('displ',varargin);
            switch k
                case 1
...
for i=1:plotstep:length(t)
    ...
    for k=1:n
        ...
        plot_sol(patch.S,getmatrixatstep(vwt_ref{patch.number},rep(i)),varargin{:});
        if ~ischarin('sigma',varargin) && ~ischarin('epsilon',varargin) && ~ischarin('energyint',varargin)
            plot_sol(interface.S,interface.P_patch*getmatrixatstep(vwt_ref{patch.number},rep(i)),'FaceColor','none','EdgeColor','k',varargin{:});
        end
    end
    if colorba
        c1 = colorbar;
    ...
    h2 = subplot(2,1,2);
    plot_sol(S_out,getmatrixatstep(vUt_ref,rep(i)),varargin{:});
    for k=1:n
        patch = patches.patches{k};
        interface = interfaces.interfaces{k};
        if ~ischarin('sigma',varargin) && ~ischarin('epsilon',varargin) && ~ischarin('energyint',varargin)
