function mov = evolSobolIndicesGlobalSolution(glob,T,Ut,alpha,varargin)
% function mov = evolSobolIndicesGlobalSolution(glob,T,Ut,alpha,varargin)
% Display evolution of the Sobol indices of global solution Ut
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Sobol index of U_' num2str(i) ' over fictitious domain for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Sobol index of U_' num2str(i) ' over fictitious domain for random variables #' num2str(alpha)])
else
    figure('Name',['Sobol index of U over fictitious domain for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Sobol index of U over fictitious domain for random variables #' num2str(alpha)])
end
clf
set(gcf,'Color','w')
...
sz = Ut.sz;
d = ndims(Ut);
sUt = SensitivityAnalysis.sobolIndices(Ut,alpha,d);
sUt = reshape(sUt,sz);
