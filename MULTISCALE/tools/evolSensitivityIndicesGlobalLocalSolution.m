function mov = evolSensitivityIndicesGlobalLocalSolution(glob,patches,interfaces,T,Ut,wt,alpha,varargin)
% function mov = evolSensitivityIndicesGlobalLocalSolution(glob,patches,interfaces,T,Ut,wt,alpha,varargin)
% Display evolution of the sensitivity indices of global solution Ut and local solution wt
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Sensitivity index of U_' num2str(i) ' and w_' num2str(i) ' over domain and patches for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Sensitivity index of U_' num2str(i) ' and w_' num2str(i) ' over domain and patches for random variables #' num2str(alpha)])
else
    figure('Name',['Sensitivity index of U and w over domain and patches for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Sensitivity index of U and w over domain and patches for random variables #' num2str(alpha)])
end
clf
set(gcf,'Color','w')
...
vUt = reshape(vUt,sz);
sUt = vUt./repmat(max_var,size(vUt,1),1);
...
swt = cellfun(@(v) v./repmat(max_var,size(v,1),1),vwt,'UniformOutput',false);
swt = cellfun(@(s) TIMEMATRIX(s,T),swt,'UniformOutput',false);
