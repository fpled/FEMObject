function mov = evolMeanLocalSolutionReference(patches,T,wt_ref,varargin)
% function mov = evolMeanLocalSolutionReference(patches,T,wt_ref,varargin)
% Display evolution of the mean (mathematical expectation) of reference local solution wt_ref
...
elseif isa(patches,'Patch')
    ...
    wt_ref{patch.number} = mean(wt_ref{patch.number});
    wt_ref{patch.number} = reshape(wt_ref{patch.number},sz);
    wt_ref{patch.number} = TIMEMATRIX(wt_ref{patch.number},T);
