function mov = evolMeanGlobalLocalSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,varargin)
% function mov = evolMeanGlobalLocalSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,varargin)
% Display evolution of the mean (mathematical expectation) of reference global solution Ut_ref and local solution wt_ref
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Mean of U_ref_' num2str(i) ' and w_ref_' num2str(i) ' over complementary subdomain and patches'])
    % set(gcf,'Name',['Mean of U_ref_' num2str(i) ' and w_ref_' num2str(i) ' over complementary subdomain and patches'])
else
    figure('Name','Mean of U_ref and w_ref over complementary subdomain and patches')
    % set(gcf,'Name','Mean of U_ref and w_ref over complementary subdomain and patches')
end
clf
set(gcf,'color','w')
...
wt_ref = cellfun(@(patch) TIMEMATRIX(reshape(mean(wt_ref{patch.number}),wt_ref{patch.number}.sz),T),patches.patches,'UniformOutput',false);
