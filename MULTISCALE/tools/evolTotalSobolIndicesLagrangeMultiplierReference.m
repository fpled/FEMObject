function mov = evolTotalSobolIndicesLagrangeMultiplierReference(interfaces,T,lambdat_ref,alpha,varargin)
% function mov = evolTotalSobolIndicesLagrangeMultiplierReference(interfaces,T,lambdat_ref,alpha,varargin)
% Display evolution of the Total Sobol indices of reference Lagrange multiplier lambdat_ref
...
if isa(interfaces,'Interfaces')
    ...
    slambdat_ref = cellfun(@(interface) TIMEMATRIX(reshape(SensitivityAnalysis.totalSobolIndices(lambdat_ref{interface.number},alpha,ndims(lambdat_ref{interface.number})),lambdat_ref{interface.number}.sz),T),interface,'UniformOutput',false);
    frame = evol_sol(T,slambdat_ref,S,'rescale',p.Results.rescale,varargin{:}); % save the frames
elseif isa(interfaces,'Interfaces')
    interface = interfaces;
    if ischarin('displ',varargin)
        i = getcharin('displ',varargin);
        figure('Name',['Total Sobol index of Lagrange multiplier lambda_ref_' num2str(i) ' over interface #' num2str(interface.number) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of Lagrange multiplier lambda_ref_' num2str(i) ' over interface #' num2str(interface.number) ' for random variables #' num2str(alpha)])
    else
        figure('Name',['Total Sobol index of Lagrange multiplier lambda_ref over interface #' num2str(interface.number) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of Lagrange multiplier lambda_ref over interface #' num2str(interface.number) ' for random variables #' num2str(alpha)])
    end
    clf
    set(gcf,'color','w')
    
    sz = lambdat_ref{interface.number}.sz;
    slambdat_ref = SensitivityAnalysis.totalSobolIndices(lambdat_ref{interface.number},alpha);
