function mov = evolGlobalLocalSolution(glob,patches,interfaces,Ut,wt,varargin)
% function mov = evolGlobalLocalSolution(glob,patches,interfaces,Ut,wt,varargin)
% Display evolution of global solution Ut and local solution wt
% glob: Global
% patches: Patches
...
if ischarin('sigma',varargin)
    ...
    figure('Name',['Global solution sig_U_' num2str(i) ' and local solution sig_w_' num2str(i) ' over domain and patches'])
    % set(gcf,'Name',['Global solution sig_U_' num2str(i) ' and local solution sig_w_' num2str(i) ' over domain and patches'])
elseif ischarin('epsilon',varargin)
    i = getcharin('epsilon',varargin);
    figure('Name',['Global solution eps_U_' num2str(i) ' and local solution eps_w_' num2str(i) ' over domain and patches'])
    % set(gcf,'Name',['Global solution eps_U_' num2str(i) ' and local solution eps_w_' num2str(i) ' over domain and patches'])
elseif ischarin('energyint',varargin)
    figure('Name',['Global solution H_U and local solution H_w over domain and patches'])
    % set(gcf,'Name',['Global solution H_U and local solution H_w over domain and patches'])
elseif ischarin('displ',varargin)
    i = getcharin('displ',varargin);
    figure('Name',['Global solution U_' num2str(i) ' and local solution w_' num2str(i) ' over domain and patches'])
    % set(gcf,'Name',['Global solution U_' num2str(i) ' and local solution w_' num2str(i) ' over domain and patches'])
elseif ischarin('rotation',varargin)
    i = getcharin('rotation',varargin);
    figure('Name',['Global solution rot_U_' num2str(i) ' and local solution rot_w_' num2str(i) ' over domain and patches'])
    % set(gcf,'Name',['Global solution rot_U_' num2str(i) ' and local solution rot_w_' num2str(i) ' over domain and patches'])
else
    figure('Name','Global solution U and local solution w over domain and patches')
...
if p.Results.rescale
    ...
    qtmin = cell(1,1+n);
    if ischarin('sigma',varargin)
        q = calc_sigma(glob.S,Ut,varargin{2:end});
        k = getcharin('sigma',varargin);
        q = sigmacompo(q,k,glob.S);
    elseif ischarin('epsilon',varargin)
        q = calc_epsilon(glob.S,Ut,varargin{2:end});
        k = getcharin('epsilon',varargin);
        q = sigmacompo(q,k,glob.S);
    elseif ischarin('energyint',varargin)
        q = calc_energyint(glob.S,Ut,varargin{2:end});
    elseif ischarin('displ',varargin)
        q = unfreevector(glob.S,Ut);
    ...
    for j=1:n
        ...
        if ischarin('sigma',varargin)
            q = calc_sigma(patch.S,wt{patch.number},varargin{2:end});
            k = getcharin('sigma',varargin);
            q = sigmacompo(q,k,patch.S);
        elseif ischarin('epsilon',varargin)
            q = calc_epsilon(patch.S,wt{patch.number},varargin{2:end});
            k = getcharin('epsilon',varargin);
            q = sigmacompo(q,k,patch.S);
        elseif ischarin('energyint',varargin)
            q = calc_energyint(patch.S,wt{patch.number},varargin{2:end});
        elseif ischarin('displ',varargin)
            q = unfreevector(patch.S,wt{patch.number});
            k = getcharin('displ',varargin);
            switch k
                case 1
...
for i=1:plotstep:length(t)
    ...
    for k=1:n
        ...
        plot_sol(patch.S,getmatrixatstep(wt{patch.number},rep(i)),varargin{:});
        if ~ischarin('sigma',varargin) && ~ischarin('epsilon',varargin) && ~ischarin('energyint',varargin)
            plot_sol(interface.S,interface.P_patch*getmatrixatstep(wt{patch.number},rep(i)),'FaceColor','none','EdgeColor','k',varargin{:});
        end
    end
    if colorba
        c1 = colorbar;
    ...
    h2 = subplot(2,1,2);
    plot_sol(glob.S,getmatrixatstep(Ut,rep(i)),varargin{:});
    for k=1:n
        patch = patches.patches{k};
        interface = interfaces.interfaces{k};
        if ~ischarin('sigma',varargin) && ~ischarin('epsilon',varargin) && ~ischarin('energyint',varargin)
