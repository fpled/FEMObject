function mov = evolClosedSobolIndicesGlobalLocalSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,alpha,varargin)
% function mov = evolClosedSobolIndicesGlobalLocalSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,alpha,varargin)
% Display evolution of the Closed Sobol indices of reference global solution Ut_ref and local solution wt_ref
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Closed Sobol index of U_ref_' num2str(i) ' and w_ref_' num2str(i) ' over complementary subdomain and patches for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Closed Sobol index of U_ref_' num2str(i) ' and w_ref_' num2str(i) ' over complementary subdomain and patches for random variables #' num2str(alpha)])
else
    figure('Name',['Closed Sobol index of U_ref and w_ref over complementary subdomain and patches for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Closed Sobol index of U_ref and w_ref over complementary subdomain and patches for random variables #' num2str(alpha)])
end
clf
set(gcf,'Color','w')
...
sUt_ref = SensitivityAnalysis.closedSobolIndices(Ut_ref,alpha,dU_ref);
sUt_ref = reshape(sUt_ref,sz);
sUt_ref = TIMEMATRIX(sUt_ref,T);
sUt_ref = unfreevector(S_out,sUt_ref)-calc_init_dirichlet(S_out)*one(T);

swt_ref = cellfun(@(patch) TIMEMATRIX(reshape(SensitivityAnalysis.closedSobolIndices(wt_ref{patch.number},alpha,ndims(wt_ref{patch.number})),wt_ref{patch.number}.sz),T),patches.patches,'UniformOutput',false);
