function mov = evolSensitivityIndicesLocalSolutionReference(patches,T,wt_ref,alpha,varargin)
% function mov = evolSensitivityIndicesLocalSolutionReference(patches,T,wt_ref,alpha,varargin)
% Display evolution of the sensitivity indices of reference local solution wt_ref
...
if isa(patches,'Patches')
    ...
    swt_ref = cellfun(@(patch) reshape(varianceConditionalExpectation(wt_ref{patch.number},alpha)./max(variance(wt_ref{patch.number})),wt_ref{patch.number}.sz),patch,'UniformOutput',false);
...
elseif isa(patches,'Patch')
    ...
    swt_ref = varianceConditionalExpectation(wt_ref{patch.number},alpha)./max(vwt_ref);
