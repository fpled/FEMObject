function mov = evolClosedSobolIndicesMultiscaleSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,alpha,varargin)
% function mov = evolClosedSobolIndicesMultiscaleSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,alpha,varargin)
% Display evolution of the Closed Sobol indices of reference multiscale solution ut_ref=(Ut_ref,wt_ref)
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Closed Sobol index of u_ref_' num2str(i) '=(U_ref_' num2str(i) ',w_ref_' num2str(i) ') over domain for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Closed Sobol index of u_ref_' num2str(i) '=(U_ref_' num2str(i) ',w_ref_' num2str(i) ') over domain for random variables #' num2str(alpha)])
else
    figure('Name',['Closed Sobol index of u_ref=(U_ref,w_ref) over domain for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Closed Sobol index of u_ref=(U_ref,w_ref) over domain for random variables #' num2str(alpha)])
end
clf
set(gcf,'color','w')
...
swt_ref = cellfun(@(patch) TIMEMATRIX(reshape(SensitivityAnalysis.closedSobolIndices(wt_ref{patch.number},alpha,ndims(wt_ref{patch.number})),wt_ref{patch.number}.sz),T),patch,'UniformOutput',false);
