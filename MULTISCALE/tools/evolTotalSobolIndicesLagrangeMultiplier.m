function mov = evolTotalSobolIndicesLagrangeMultiplier(interfaces,T,lambdat,alpha,varargin)
% function mov = evolTotalSobolIndicesLagrangeMultiplier(interfaces,T,lambdat,alpha,varargin)
% Display evolution of the Total Sobol indices of Lagrange multiplier lambdat
...
if isa(interfaces,'Interfaces')
    ...
    if ischarin('displ',varargin)
        ...
        figure('Name',['Total Sobol index of lambda_' num2str(i) ' over interfaces #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of lambda_' num2str(i) ' over interfaces #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
    else
        figure('Name',['Total Sobol index of lambda over interfaces #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of lambda over interfaces #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
    end
    clf
    set(gcf,'Color','w')
    ...
    slambdat = cellfun(@(interface) TIMEMATRIX(reshape(SensitivityAnalysis.totalSobolIndices(lambdat{interface.number},alpha,ndims(lambdat{interface.number})),lambdat{interface.number}.sz),T),interface,'UniformOutput',false);
    frame = evol_sol(T,slambdat,S,'rescale',p.Results.rescale,varargin{:}); % save the frames
elseif isa(interfaces,'Interface')
    interface = interfaces;
    if ischarin('displ',varargin)
        i = getcharin('displ',varargin);
        figure('Name',['Total Sobol index of Lagrange multiplier lambda_' num2str(i) ' over interface #' num2str(interface.number) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of Lagrange multiplier lambda_' num2str(i) ' over interface #' num2str(interface.number) ' for random variables #' num2str(alpha)])
    else
        figure('Name',['Total Sobol index of Lagrange multiplier lambda over interface #' num2str(interface.number) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of Lagrange multiplier lambda over interface #' num2str(interface.number) ' for random variables #' num2str(alpha)])
    end
    clf
    set(gcf,'Color','w')
    
    sz = lambdat{interface.number}.sz;
    slambdat = SensitivityAnalysis.totalSobolIndices(lambdat{interface.number},alpha);
