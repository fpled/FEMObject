function mov = evolTotalSobolIndicesLocalSolution(patches,T,wt,alpha,varargin)
% function mov = evolTotalSobolIndicesLocalSolution(patches,T,wt,alpha,varargin)
% Display evolution of the Total Sobol indices of local solution wt
...
if isa(patches,'Patches')
    ...
    if ischarin('displ',varargin)
        ...
        figure('Name',['Total Sobol index of w_' num2str(i) ' over patches #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of w_' num2str(i) ' over patches #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
    else
        figure('Name',['Total Sobol index of w over patches #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of w over patches #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
    end
    clf
    set(gcf,'Color','w')
    ...
    swt = cellfun(@(patch) TIMEMATRIX(reshape(SensitivityAnalysis.totalSobolIndices(wt{patch.number},alpha,ndims(wt{patch.number})),wt{patch.number}.sz),T),patch,'UniformOutput',false);
...
elseif isa(patches,'Patch')
    ...
    if ischarin('displ',varargin)
        i = getcharin('displ',varargin);
        figure('Name',['Total Sobol index of w_' num2str(i) ' over patch #' num2str(patch.number) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of w_' num2str(i) ' over patch #' num2str(patch.number) ' for random variables #' num2str(alpha)])
    else
        figure('Name',['Total Sobol index of w over patch #' num2str(patch.number) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Total Sobol index of w over patch #' num2str(patch.number) ' for random variables #' num2str(alpha)])
    end
    clf
    set(gcf,'Color','w')
    
    sz = wt{patch.number}.sz;
    swt = SensitivityAnalysis.totalSobolIndices(wt{patch.number},alpha);
    swt = reshape(swt,sz);
