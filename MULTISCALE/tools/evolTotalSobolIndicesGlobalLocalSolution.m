function mov = evolTotalSobolIndicesGlobalLocalSolution(glob,patches,interfaces,T,Ut,wt,alpha,varargin)
% function mov = evolTotalSobolIndicesGlobalLocalSolution(glob,patches,interfaces,T,Ut,wt,alpha,varargin)
% Display evolution of the Total Sobol indices of global solution Ut and local solution wt
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Total Sobol index of U_' num2str(i) ' and w_' num2str(i) ' over domain and patches for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Total Sobol index of U_' num2str(i) ' and w_' num2str(i) ' over domain and patches for random variables #' num2str(alpha)])
else
    figure('Name',['Total Sobol index of U and w over domain and patches for random variables #' num2str(alpha)])
...
swt = cellfun(@(patch) TIMEMATRIX(reshape(SensitivityAnalysis.totalSobolIndices(wt{patch.number},alpha,ndims(wt{patch.number})),wt{patch.number}.sz),T),patches.patches,'UniformOutput',false);
