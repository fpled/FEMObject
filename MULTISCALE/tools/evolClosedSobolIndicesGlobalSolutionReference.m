function mov = evolClosedSobolIndicesGlobalSolutionReference(glob,T,Ut_ref,alpha,varargin)
% function mov = evolClosedSobolIndicesGlobalSolutionReference(glob,T,Ut_ref,alpha,varargin)
% Display evolution of the Closed Sobol indices of reference global solution Ut_ref
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Closed Sobol index of U_ref_' num2str(i) ' over complementary subdomain for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Closed Sobol index of U_ref_' num2str(i) ' over complementary subdomain for random variables #' num2str(alpha)])
else
    figure('Name',['Closed Sobol index of U_ref over complementary subdomain for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Closed Sobol index of U_ref over complementary subdomain for random variables #' num2str(alpha)])
end
clf
set(gcf,'Color','w')
...
sz = Ut_ref.sz;
d = ndims(Ut_ref);
sUt_ref = SensitivityAnalysis.closedSobolIndices(Ut_ref,alpha,d);
