function mov = evolTotalSobolIndicesMultiscaleSolution(glob,patches,interfaces,T,Ut,wt,alpha,varargin)
% function mov = evolTotalSobolIndicesMultiscaleSolution(glob,patches,interfaces,T,Ut,wt,alpha,varargin)
% Display evolution of the Total Sobol indices of multiscale solution ut=(Ut,wt)
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Total Sobol index of u_' num2str(i) '=(U_' num2str(i) ',w_' num2str(i) ') over domain for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Total Sobol index of u_' num2str(i) '=(U_' num2str(i) ',w_' num2str(i) ') over domain for random variables #' num2str(alpha)])
else
    figure('Name',['Total Sobol index of u=(U,w) over domain for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Total Sobol index of u=(U,w) over domain for random variables #' num2str(alpha)])
end
clf
set(gcf,'color','w')
...
sz = Ut.sz;
dUt = ndims(Ut);
sUt = SensitivityAnalysis.totalSobolIndices(Ut,alpha,dUt);
sUt = reshape(sUt,sz);
