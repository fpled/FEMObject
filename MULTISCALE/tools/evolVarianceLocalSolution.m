function mov = evolVarianceLocalSolution(patches,T,wt,varargin)
% function mov = evolVarianceLocalSolution(patches,T,wt,varargin)
% Display evolution of the variance of local solution wt
...
if isa(patches,'Patches')
    ...
    frame = evol_sol(T,vwt,S,'rescale',p.Results.rescale,varargin{:}); % save the frames
elseif isa(patches,'Patch')
    ...
    vwt = reshape(vwt,sz);
    vwt = TIMEMATRIX(vwt,T);
    
    vwt = setevolparam(vwt,'colormap',p.Results.colormap,'colorbar',p.Results.colorbar,...
        'view',p.Results.view,'camup',p.Results.camup,'campos',p.Results.campos,'FontSize',p.Results.FontSize);
    frame = evol_sol(vwt,patch.S,'rescale',p.Results.rescale,varargin{:}); % save the frames
end
