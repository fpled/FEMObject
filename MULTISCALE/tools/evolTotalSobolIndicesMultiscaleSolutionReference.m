function mov = evolTotalSobolIndicesMultiscaleSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,alpha,varargin)
% function mov = evolTotalSobolIndicesMultiscaleSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,alpha,varargin)
% Display evolution of the Total Sobol indices of reference multiscale solution ut_ref=(Ut_ref,wt_ref)
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Total Sobol index of u_ref_' num2str(i) '=(U_ref_' num2str(i) ',w_ref_' num2str(i) ') over domain for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Total Sobol index of u_ref_' num2str(i) '=(U_ref_' num2str(i) ',w_ref_' num2str(i) ') over domain for random variables #' num2str(alpha)])
else
    figure('Name',['Total Sobol index of u_ref=(U_ref,w_ref) over domain for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Total Sobol index of u_ref=(U_ref,w_ref) over domain for random variables #' num2str(alpha)])
end
clf
set(gcf,'color','w')
...
sUt_ref = SensitivityAnalysis.totalSobolIndices(Ut_ref,alpha,dU_ref);
...
swt_ref = cellfun(@(patch) TIMEMATRIX(reshape(SensitivityAnalysis.totalSobolIndices(wt_ref{patch.number},alpha,ndims(wt_ref{patch.number})),wt_ref{patch.number}.sz),T),patch,'UniformOutput',false);
