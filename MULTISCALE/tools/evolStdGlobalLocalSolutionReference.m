function mov = evolStdGlobalLocalSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,varargin)
% function mov = evolStdGlobalLocalSolutionReference(glob,patches,interfaces,T,Ut_ref,wt_ref,varargin)
% Display evolution of the standard deviation of reference global solution Ut_ref and local solution wt_ref
...
swt_ref = cellfun(@(patch) TIMEMATRIX(reshape(std(wt_ref{patch.number}),wt_ref{patch.number}.sz),T),patches.patches,'UniformOutput',false);
