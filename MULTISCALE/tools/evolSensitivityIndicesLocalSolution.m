function mov = evolSensitivityIndicesLocalSolution(patches,T,wt,alpha,varargin)
% function mov = evolSensitivityIndicesLocalSolution(patches,T,wt,alpha,varargin)
% Display evolution of the sensitivity indices of local solution wt
...
if isa(patches,'Patches')
    ...
    swt = cellfun(@(patch) reshape(varianceConditionalExpectation(wt{patch.number},alpha)./max(variance(wt{patch.number})),wt{patch.number}.sz),patch,'UniformOutput',false);
...
elseif isa(patches,'Patch')
    ...
    vwt = variance(wt{patch.number});
    swt = varianceConditionalExpectation(wt{patch.number},alpha)./max(vwt);
    swt = reshape(swt,sz);
